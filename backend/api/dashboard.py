...
async def get_dashboard_summary(user = Depends(get_current_user)):
    try:
        ...
        active_challenges = [Challenge(**c) for c in challenges if not c.get("is_completed")]
        ...
        if latest_entry:
            return DashboardSummary(
                ...
                impact_equivalents=calculate_equivalents(latest_entry["total_emissions"]),
...
async def complete_challenge(id: str, user = Depends(get_current_user)):
    try:
        ...
        if profile and updated:
            ...
            if new_points >= 50 and not any(b["badge_name"] == "Eco Warrior" for b in BadgeRepository.get_all(user.id)):
