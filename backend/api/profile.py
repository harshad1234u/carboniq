...
async def signup(auth_data: AuthSignup):
    client = get_supabase()
    try:
...
async def login(auth_data: AuthLogin):
    client = get_supabase()
    try:
...
async def update_user_profile(profile_data: ProfileCreate, user = Depends(get_current_user)):
    try:
