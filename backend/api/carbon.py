...
from database.repositories import CarbonRepository, RecommendationRepository, EcoPredictionRepository
...
async def calculate_carbon(input_data: CarbonInput, user = Depends(get_current_user)):
    try:
        # Calculate emissions
...
async def get_city_weather(city: str, user = Depends(get_current_user)):
    weather = await get_weather(city)
    return weather

@router.post("/ai-coach", response_model=AiCoachResponse)
async def run_ai_coach(user = Depends(get_current_user)):
    try:
        ...
        RecommendationRepository.create({
            ...
            "weather_context": response.weather_context.model_dump() if response.weather_context else None,
...
async def run_eco_twin(request: EcoTwinRequest, user = Depends(get_current_user)):
    try:
        latest_entry = CarbonRepository.get_latest(user.id)
        if not latest_entry:
            raise HTTPException(status_code=400, detail="No carbon entry found")
        ...
        if not latest_recs:
            raise HTTPException(status_code=400, detail="No recommendations found. Run AI Coach first.")
        ...
        raise HTTPException(status_code=500, detail="Failed to generate Eco Twin prediction")
