...
def test_missing_profile_after_auth(mocker):
    ...
    response = client.post(
        "/api/auth/login", json={"email": "test@test.com", "password": "pwd"}
    )
...
def test_invalid_auth_credentials(mocker):
    ...
    response = client.post(
        "/api/auth/login", json={"email": "wrong@test.com", "password": "bad"}
    )
...
def test_expired_session(mocker):
    ...
    response = client.get(
        "/api/profile", headers={"Authorization": "Bearer expired_token"}
    )
