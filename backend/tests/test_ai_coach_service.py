import pytest
from services.ai_coach_service import get_recommendations, _select_fallback_recommendations
...
async def test_fallback_recommendations_no_key():
    ...
    assert "AC" in response.recommendations[0].title or "LED" in response.recommendations[0].title or "Appliances" in response.recommendations[0].title

@pytest.mark.asyncio
async def test_fallback_recommendations_exception(mocker):
    mocker.patch("services.ai_coach_service._call_gemini", side_effect=Exception("API Down"))
    footprint = _get_mock_footprint()
    response = await get_recommendations(footprint, None, {})
    assert len(response.recommendations) == 3
...
async def test_gemini_success(mocker):
    ...
    mock_recs = [
        Recommendation(title="Test", description="Test Desc", co2_savings_kg=10, cost_savings_inr=100, difficulty="easy"),
        Recommendation(title="Test 2", description="Test Desc 2", co2_savings_kg=20, cost_savings_inr=200, difficulty="medium"),
        Recommendation(title="Test 3", description="Test Desc 3", co2_savings_kg=30, cost_savings_inr=300, difficulty="hard")
    ]
