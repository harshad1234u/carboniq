...
def test_rls_violations():
    ...
    # Create User A
    try:
        user_a_res = admin_client.auth.sign_up(
            {
                "email": user_a_email,
                "password": password,
        ...
        # Create User B
        user_b_res = admin_client.auth.sign_up(
            {
                "email": user_b_email,
                "password": password,
        ...
        user_b_client = create_client(
            settings.supabase_url,
            os.environ.get("SUPABASE_ANON_KEY", settings.supabase_service_role_key),
        )
        ...
        result = (
            user_b_client.table("profiles").select("*").eq("id", user_a_id).execute()
        )
