...
class TestFallbackSelection:
    ...
    def test_transport_highest_gives_transport_recs(self):
        ...
        assert any(
            kw in recs[0].title.lower()
            for kw in ["transport", "walk", "carpool", "cycle", "drive"]
        )
    ...
    def test_flights_highest_gives_flight_rec(self):
        ...
        assert len(recs) == 3
        assert any(
            "flight" in r.title.lower() or "train" in r.title.lower() for r in recs
        )
...
class TestGetRecommendationsIntegration:
    ...
    async def test_savings_totals_are_correct(self):
        ...
        try:
            ...
            expected_cost = round(
                sum(r.cost_savings_inr for r in resp.recommendations), 2
            )
